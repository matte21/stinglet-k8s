...
func AddKubeletConfigFlags(mainfs *pflag.FlagSet, c *kubeletconfig.KubeletConfiguration) {
	...
	fs.StringVar(&c.TopologyManagerPolicy, "topology-manager-policy", c.TopologyManagerPolicy, "Topology Manager policy to use. Possible values: 'none', 'best-effort', 'restricted', 'single-numa-node', 'unifiedFarMem.")
	...
	fs.StringVar(&c.NUMALatencyMatrixFile, "numa-latency-matrix-file", c.NUMALatencyMatrixFile, "The path to the file holding the NUMA latency matrix, used by the far memory manager to learn the distances between the NUMA nodes.")
}
